...
*   **Cơ chế Đăng nhập một lần (Single Sign-On):** Người dùng chỉ cần nhập mật khẩu một lần duy nhất lúc khởi động ứng dụng để mở khóa kho khóa Keystore PKCS12 và trích xuất private key định danh cá nhân tại thiết bị của mình. Giao thức đăng nhập một lần được thực hiện như sau:
    1.  *Xác thực ban đầu:* Thiết bị của người dùng ký số lên dữ liệu thách thức bằng private key của mình rồi gửi lên máy chủ dịch vụ xác thực Authentication Service để yêu cầu cấp vé nhận dạng Ticket Granting Ticket. Sau khi máy chủ xác thực thành công, người dùng nhận về vé nhận dạng Ticket Granting Ticket và session key trung gian. Cả hai thông tin này được lưu trữ vào bộ nhớ đệm Ticket Cache cục bộ.
    2.  *Yêu cầu kết nối dịch vụ không cần mật khẩu:* Khi người dùng muốn kết nối đến các dịch vụ khác nhau trong hệ thống (như Chat Server hoặc Notification Server), ứng dụng sẽ tự động trích xuất vé nhận dạng Ticket Granting Ticket và session key trung gian từ bộ nhớ đệm mà không yêu cầu người dùng nhập lại mật khẩu hay thực hiện lại các phép toán ký số bất đối xứng RSA phức tạp.
    3.  *Cấp vé dịch vụ tự động:* Ứng dụng gửi vé nhận dạng Ticket Granting Ticket và thẻ xác thực Authenticator được mã hóa bằng session key trung gian lên máy chủ cấp vé Ticket Granting Service. Máy chủ kiểm tra tính hợp lệ và phản hồi vé dịch vụ Service Ticket tương ứng với quyền hạn của người dùng, giúp người dùng truy cập liền mạch các dịch vụ mà chỉ cần đăng nhập một lần duy nhất.
