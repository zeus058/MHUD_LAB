...
*   **Cơ chế Đăng nhập một lần (Single Sign-On - SSO) hiệu năng cao:** Người dùng chỉ cần nhập mật khẩu một lần duy nhất lúc khởi động ứng dụng để mở khóa Keystore PKCS12 và trích xuất private key cục bộ. Kịch bản SSO diễn ra như sau:
    1.  *Xác thực ban đầu (Initial Authentication):* Client ký số thách thức bằng private key của mình gửi lên cổng AS để xin vé TGT. Sau khi AS xác thực thành công, Client nhận về vé TGT và session key trung gian $K_{Client-TGS}$. Cả hai dữ liệu này được lưu trữ vào bộ nhớ đệm an toàn cục bộ (**Ticket Cache**).
    2.  *Yêu cầu dịch vụ không cần mật khẩu:* Khi Client muốn truy cập vào bất kỳ dịch vụ nào trong hệ thống (như Chat Server hay Notification Server), nó **không yêu cầu người dùng phải nhập lại mật khẩu** và cũng **không phải thực hiện các phép toán bất đối xứng RSA/ECDSA đắt đỏ** nữa.
    3.  *Cấp vé tự động:* Client chỉ cần trích xuất vé TGT từ Ticket Cache, tạo một Authenticator mã hóa bằng session key $K_{Client-TGS}$ đã lưu để gửi lên cổng TGS. TGS kiểm tra tính hợp lệ của TGT và lập tức cấp vé dịch vụ ST tương ứng với phân quyền của người dùng. Giao thức này cho phép mở rộng không giới hạn số lượng dịch vụ nội bộ mà trải nghiệm của người dùng vẫn liền mạch (chỉ đăng nhập 1 lần duy nhất).
